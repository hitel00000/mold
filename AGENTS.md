...
* **실행 코드 변경 시 (Go, TS, SQL 등)**:
  기능 구현, 버그 수정, 리팩토링 등 실제 동작 코드에 변경이 있을 때는
  반드시 `go test -count=1 ./...` (또는 해당 타깃 테스트)를 fresh하게
  실행하여 100% 무결성을 실측 검증한다.
* **문서 전용 변경 시 (Docs-only changes)**:
  `*.md` 마크다운 문서(`NOW.md`, `TASKS.md`, `docs/*`, `README.md`, `AGENTS.md` 등)나
  단순 주석/텍스트 수정처럼 **실제 실행 코드 및 바이너리에 전혀 영향이 없는
  수정의 경우, 불필요하게 무거운 빌드나 전체 테스트 스위트를 돌리지 않는다.**
  (불필요한 리소스 및 턴 소모 방지).
* **혼합 변경 시 (코드 + 문서 동시 수정)**:
  실행 코드가 1줄이라도 포함되어 있다면 반드시 테스트를 실행하여 검증한다.
...
* `NOW.md` 및 `TASKS.md`는 세션 간 핸드오프 및 진척 관리를 위한 문서이다.
  **`NOW.md` 및 `TASKS.md`의 완료 상태 갱신 커밋은 반드시 리뷰어(사용자)의
  최종 승인 코멘트 이후에만 생성한다** (구현 커밋이나 최초 PR/보고 사이클에
  함께 포함시키는 것을 엄격히 금지함).
