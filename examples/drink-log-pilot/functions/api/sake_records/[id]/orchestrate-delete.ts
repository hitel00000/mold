...
// Guarantees Abort Contract and Idempotent Retry
...
export async function onRequestDelete(context: { request: Request; params: { id: string }; env: Env }) {
  ...
  // 4. Sequential Child Images Deletion
  const urlOrigin = new URL(request.url).origin;
  for (const img of childImages) {
    // Step A: Delete R2 Blob (via HTTP API or R2 bucket directly)
    if (img.image_key) {
      try {
        await env.BUCKET.delete(img.image_key);
      } catch (e) {
    ...
    // Step B: Delete SakeImage Record
  ...
  // 6. Delete SakeRecord Parent via HTTP API
