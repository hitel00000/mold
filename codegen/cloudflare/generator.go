...
`)
	...
	for _, res := range resources {
		...
		sb.WriteString("      const jsonPart = formData.get('payload') || formData.get('data');\n")
		sb.WriteString("      if (jsonPart && typeof jsonPart === 'string') {\n")
		sb.WriteString("        try {\n")
		sb.WriteString("          const parsed = JSON.parse(jsonPart);\n")
		sb.WriteString("          if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)) {\n")
		sb.WriteString("            Object.assign(body, parsed);\n")
		sb.WriteString("          }\n")
		sb.WriteString("        } catch (e) {\n")
		sb.WriteString("          return writeError(c, 400, 'INVALID_JSON', 'failed to parse JSON payload in multipart form');\n")
		sb.WriteString("        }\n")
		sb.WriteString("      }\n")
		sb.WriteString("      formData.forEach((val, key) => {\n")
		sb.WriteString("        if (key !== 'payload' && key !== 'data' && typeof val === 'string') {\n")
		sb.WriteString("          body[key] = (val !== '' && !isNaN(Number(val))) ? Number(val) : val;\n")
		sb.WriteString("        }\n")
		sb.WriteString("      });\n")
		sb.WriteString("    } catch (e) {\n")
		...
		if len(hasManyRels) > 0 {
			...
			for _, rel := range hasManyRels {
				...
				sb.WriteString(fmt.Sprintf("    if (typeof body['%s'] === 'string') {\n", rel.Name))
				sb.WriteString(fmt.Sprintf("      try { body['%s'] = JSON.parse(body['%s']); } catch (e) {}\n", rel.Name, rel.Name))
				sb.WriteString("    }\n")
		...
		if hasBlob {
			...
			for _, f := range p.Fields {
				if f.Type == resource.TypeBlob && !f.Deprecated {
					...
					sb.WriteString(fmt.Sprintf("      let targetFile_%s: any = file_%s;\n", blobField, blobField))
					if blobField != "file" {
						sb.WriteString(fmt.Sprintf("      if ((targetFile_%s === null || targetFile_%s === undefined || targetFile_%s === '') && formData.get('file')) {\n", blobField, blobField, blobField))
						sb.WriteString(fmt.Sprintf("        targetFile_%s = formData.get('file');\n", blobField))
						sb.WriteString("      }\n")
					}
					sb.WriteString(fmt.Sprintf("      if (targetFile_%s !== null && targetFile_%s !== undefined && targetFile_%s !== '') {\n", blobField, blobField, blobField))
					sb.WriteString(fmt.Sprintf("        let fileData_%s: any = targetFile_%s;\n", blobField, blobField))
					sb.WriteString(fmt.Sprintf("        let mimeType_%s = 'application/octet-stream';\n", blobField))
					sb.WriteString(fmt.Sprintf("        let ext_%s = '';\n", blobField))
					sb.WriteString(fmt.Sprintf("        if (typeof targetFile_%s === 'object' && targetFile_%s !== null) {\n", blobField, blobField))
					sb.WriteString(fmt.Sprintf("          mimeType_%s = (targetFile_%s as any).type || mimeType_%s;\n", blobField, blobField, blobField))
					sb.WriteString(fmt.Sprintf("          if ((targetFile_%s as any).name) { ext_%s = (targetFile_%s as any).name.substring((targetFile_%s as any).name.lastIndexOf('.')); }\n", blobField, blobField, blobField, blobField))
					sb.WriteString(fmt.Sprintf("          if (typeof (targetFile_%s as any).arrayBuffer === 'function') {\n", blobField))
					sb.WriteString(fmt.Sprintf("            fileData_%s = await (targetFile_%s as any).arrayBuffer();\n", blobField, blobField))
					sb.WriteString(fmt.Sprintf("          } else if (typeof (targetFile_%s as any).stream === 'function') {\n", blobField))
					sb.WriteString(fmt.Sprintf("            fileData_%s = (targetFile_%s as any).stream();\n", blobField, blobField))
					sb.WriteString("          }\n")
					sb.WriteString("        }\n")
		...
		for _, f := range p.Fields {
			...
			sb.WriteString(fmt.Sprintf("  let file = formData ? formData.get('%s') as File : null;\n", blobField))
			if blobField != "file" {
				sb.WriteString("  if (!file && formData) {\n")
				sb.WriteString("    file = formData.get('file') as File;\n")
				sb.WriteString("  }\n")
			}
			...
			sb.WriteString("  let fileData: any = file;\n")
			sb.WriteString("  if (typeof (file as any).arrayBuffer === 'function') {\n")
			sb.WriteString("    fileData = await (file as any).arrayBuffer();\n")
			sb.WriteString("  } else if (typeof (file as any).stream === 'function') {\n")
			sb.WriteString("    fileData = (file as any).stream();\n")
			sb.WriteString("  }\n")
			sb.WriteString("  await c.env.BUCKET.put(key, fileData, { httpMetadata: { contentType: file.type } });\n")
