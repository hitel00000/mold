...
**완료된 마일스톤**: Milestone 0(철학 고정), Milestone 1(Resource IR), Milestone 2(Storage/SQLite Adapter), Milestone 3(Transport/REST API), Milestone 4(Default View/HTML CRUD), Milestone 5(Identity/Auth & Session & Permissions), Milestone 6(AI Workflow & Resource Guide & Zero-Code Service Expansion)  
👉 **Mold MVP 100% 개발 완결**

**진행 중인 마일스톤**: 없음 (MVP 전체 완료)

**다음 시작점 (Post-MVP)**:
1. PostgreSQL / MySQL DB Storage Adapter 확장
2. Schema Diff 기반 Non-destructive Migration 지원
3. FK Form UI Select Dropdown 확장
...
- **AI Workflow & Zero-Code Expansion**: Go 코드 수정 0줄로 Resource YAML만 추가/수정 후 `POST /_mold/reload` (admin 세션) 호출 시 DB Table, REST API, HTML View가 원자적으로 자동 반영.
- **Resource Guide**: `docs/resource-guide.md`에 Good/Bad 패턴 대조 가이드를 작성하여 AI의 오설정 방지.
...
Post-MVP 확장 필요성 논의 및 PostgreSQL 어댑터 등 확장 작업 진행 시 `docs/retrospectives/milestone-6.md` 회고 문서 참고.
...
- 2026-07-22: Milestone 6 (AI Workflow) 및 MVP 전체 100% 완료 반영
