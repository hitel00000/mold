...
**완료된 마일스톤**: Milestone 0(철학 고정), Milestone 1(Resource IR), Milestone 2(Storage/SQLite Adapter)

**진행 중인 마일스톤**: 없음 (Milestone 2 종료 후 휴지 상태)

**다음 시작점**: Milestone 3 — Transport (REST API)
...
- **Auth 방향**: 세션 쿠키 기반으로 갈 예정 (아직 미구현, Milestone 5)
- **reload 트리거**: 파일 워처 대신 명시적 API (`POST /_mold/reload`, admin 세션 필요) — 결정성 확보 목적
- **프로젝트 포지셔닝**: 복잡한 프로덕션 서비스가 아니라, 빠른 프로토타이핑/작은 프로덕트용 도구

---

## 다음 할 일: Milestone 3 (Transport)

착수 전에 `docs/retrospectives/milestone-2.md`의 "다음 마일스톤에 적용할
체크리스트" 5개 항목을 먼저 훑어볼 것. Milestone 2에서 반복됐던 문제
패턴(타입 검증 순서, 필드 화이트리스트, 조건부 로직 전수 반영, 속성 조합
엣지 케이스)이 Transport에서도 그대로 재현될 가능성이 높으므로, 이번엔
처음 구현 단계부터 반영해서 리뷰 사이클을 줄이는 게 목표.

Milestone 3 범위 (TASKS.md 기준): HTTP Router, List/Detail/Create/Update/Delete
5개 엔드포인트, Pagination. 완료 기준은 "새로운 Resource를 추가하면 API가
자동으로 생긴다".
...
- 없음 (Milestone 2까지 나온 질문은 전부 정리됨)
